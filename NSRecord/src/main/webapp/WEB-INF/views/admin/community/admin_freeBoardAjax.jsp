				...
				<c:choose>
					<c:when test="${fn:length(freeBoardList) > 0 }">
						<tr>
							...
							<th class="boardCol" tabindex="0" aria-controls="adminNoticeBoard" rowspan="1" colspan="1"
								aria-label="CSS grade: activate to sort column ascending">상태</th>	
						</tr>
					</c:when>
					<c:when test="${fn:length(freeBoardList) <= 0 }">
						<td>등록된 게시글이 없습니다.</td>
...
<div class="row flexBox" style="padding: 0 10px;">
	<div class="col-sm-5">
		<div class="dataTables_info" id="example1_info" role="status" aria-live="polite">
			<button type="button" class="btn btn-primary" onclick="javascript:location.href='<%=contextPath%>/adminCommunity/adminFreeBoardWriter'">글쓰기</button>
		</div>
	</div>	
	<div class="col-sm-7">
